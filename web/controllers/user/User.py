...
def login():
    ...
    if request.method == 'GET':
        ...
        # 用户名和密码校验
        if login_name is None or len(login_name) < 3:
            response["code"] = -1
            response["msg"] = "登录失败，请输入正确用户名和密码"
        ...
        response.set_cookie(app.config['AUTH_COOKIE_NAME'], "{}#{}".format(UserService.geneAuthCode(user_info), user_info.uid))
...
def edit():
    ...
    if nickname is None or len(nickname) < 1:
        response['code'] = -1
        response['msg'] = "请输入符合规范的姓名"
        return jsonify(response)

    if email is None or len(email) < 1:
        response['code'] = -1
        response['msg'] = "请输入符合规范的邮箱"
        return jsonify(response)
